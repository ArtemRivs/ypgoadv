...
// Эта программа демонстрирует использование объекта типа `Cond` из пакета `sync`.
// Она создает объект `State`, который содержит флаг `ready` и объект `Cond`, используемый
// для ожидания, пока флаг будет установлен.
// Затем она запускает одну анонимную горутину, которая устанавливает флаг `ready` в `true`
// после задержки в 500 миллисекунд, и пять горутин, которые ждут, пока флаг будет установлен,
// а затем выводят сообщение "ready!". Когда горутины закончили выполнение, программа завершается.
